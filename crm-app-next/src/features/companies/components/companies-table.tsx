...
export function CompaniesTable() {
  ...
  const [industryFilter, setIndustryFilter] = useQueryState("industry", parseAsString.withDefault("all"));
  ...
  const queryParams = useMemo<SearchParams>(() => {
    ...
    return params;
  }, [page, searchParam, industryFilter, ownerFilter, sorting]);
  ...
  const columns = useMemo(
    () => [
      ...
      ...getCompanyColumns({
        ...
        onView: (c) => router.push(`/companies/${c.id}`),
        users,
      }),
    ],
    [router, users]
  ...
  const handleBulkExport = () => {
    ...
    const headers = ["Name", "Domain", "Industry", "Size", "Phone", "Website", "Owner"];
    const csv = [
      headers.join(","),
      ...rows.map((c) =>
        [c.name, c.domain ?? "", c.industry ?? "", c.employeeCount ?? "", c.phone ?? "", c.website ?? "", c.ownerId]
          .map((v) => `"${v}"`)
          .join(",")
      ),
    ].join("\n");
    ...
    toast.success(`Exported ${rows.length} company(ies)`);
  };
  ...
  const industryLabel = INDUSTRY_FILTERS.find((o) => o.value === industryFilter)?.label ?? "All Industries";
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-1 flex-wrap items-center gap-2">
          ...
          <Select
            ...
            onValueChange={(val) => startTransition(() => { setIndustryFilter(val ?? "all"); setPage(1); })}
          >
      ...
      <ConfirmDialog
        ...
        description={`Are you sure you want to delete "${deleteCompany?.name}"? Associated contacts and deals will NOT be deleted, but their company link will be removed.`}
        confirmLabel="Delete"
        variant="destructive"
